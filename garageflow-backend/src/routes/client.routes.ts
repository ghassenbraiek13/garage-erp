...
r.get(
  ...
  ctrl.exportCarnetPDF,
)
r.get('/:id', verifyToken, requireRole('manager', 'cashier', 'superadmin'), ctrl.getOne)
r.post('/', verifyToken, requireRole('manager', 'mechanic', 'cashier', 'superadmin'), validateBody(CreateClientSchema), ctrl.create)
...
r.get('/:id/vehicles', verifyToken, requireRole('manager', 'mechanic', 'cashier', 'superadmin'), ctrl.vehicles)
